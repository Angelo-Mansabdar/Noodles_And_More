    ...
    <div class="max-w-7xl mx-auto px-6 py-8">
        ...
        <form action="{{ route('products.index') }}" method="GET" class="flex justify-center mb-6">
            <input 
                ...
                class="p-2 border rounded-l-md focus:outline-none" 
        ...
        @if($products->count() > 0)
            <div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-6">
                @foreach ($products as $product)
                    <a href="{{ route('products.show', $product->id) }}" class="block bg-white shadow-md rounded-lg overflow-hidden hover:shadow-lg transition-shadow duration-300">
